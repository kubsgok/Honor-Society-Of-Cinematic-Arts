...
export const NavBar = () => {
  ...
  return (
    <div className="relative w-full h-[90px] bg-[#520392] flex items-center shadow-md">
      {/* Logo Section */}
      <div
        className="flex-shrink-0 mr-8 cursor-pointer"
        onClick={() => router.push("/")}
      >
        <Image
          width={75}
      ...
      <div className="flex space-x-6">
        ...
        <button
          ...
          onClick={() => router.push("/educational-resources")}
        >
          {userType !== "Associate" && userType !== "Nominee For Induction" ? (
            <>
        ...
        {userType === "Admin" && (
          <button
            className={`group relative text-[21px] font-bold text-white px-5 py-2.5 rounded-xl cursor-pointer select-none transition-all duration-200 ease-out focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/30 ${
              currentPath == "/staff-interface" ? "bg-white/15" : "bg-transparent hover:bg-white/10"
            } hover:-translate-y-0.5 active:translate-y-0`}
        ...
        {(userType === "Admin" || userType === "Chapter Director") && (
          <button
            className={`group relative text-[21px] font-bold text-white px-5 py-2.5 rounded-xl cursor-pointer select-none transition-all duration-200 ease-out focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/30 ${
              currentPath == "/billing" ? "bg-white/15" : "bg-transparent hover:bg-white/10"
            } hover:-translate-y-0.5 active:translate-y-0`}
      ...
      <div className="absolute right-10 flex items-center space-x-4">
        {/* Mail Icon - only show to Admin and Chapter Director */}
        {(userType === "Admin" || userType === "Chapter Director") && (
